...
Load packages and data that going to be used. I aslo defined a theme function for customized ggplots.
...
By looking at the discriptive statistics of the dataset, we find that *NeutralField* is constant to 0. We can remove this variable from dataset. Current score, current red cards and remaining goals of both teams are calculated from the raw dataset.
...
Clearly we saw high correlations between in-game stats such as SoG, DAT, CR, etc. In order to reduce the risk of overfitting, let's run a principal component analysis on the dataset.
...
From the PCA result we can see that PC1 and PC2 together contributed 71% of the total variance. Elbow method also suggeset that in this case choosing the first two principe component seems to be the most appropriate choice. Therefore, the variables I choosed to enter the regression model are **MeanA&H, Minute, PC1, PC2,$\lambda_{xy}$**. For Scaling proporses, **Minute** will be rescaled to [0,1] and will be taken square root in order to amplify the minute effect in the early game. Hence, our poisson regression link functions for home team and away team became

$$log(\lambda_{k,t}) = \beta_0*\sqrt{T} + \beta_1*Mean_H + \beta_2*PC1_H + \beta_3*PC2_H + \lambda_{xy} + \beta_4$$
$$log(\mu_{k,t}) = \gamma_0*T^2 + \gamma_1*Mean_A + \gamma_2*PC1_A + \gamma_3*PC2_A + \mu_{xy} + \gamma_4$$
...
Before training the model, I split the whole dataset into test and training with 60% and 40% of the data respectively. Note that the sampling is not happening at minutes level but at the match level instead in order to make sure that every match has full 91 minutes. Principle components are calculated using *preProcess()* function of *caret* package.
...
In my above code I filtered out Minute = 0 because it is not the actually minutes happened in the game and if kept will zero-inflated the data.
...
Function *nlm()* will try to minimize the input function using a Newton-type algorithm. therefore I took the negative of likelihood function in order to achieve MLE estimate. 

This process takes about 3 minutes to run and in the output of *nlm()*, code = 1 means relative gradient is close to zero, current iterate is probably solution. Array **par** is our parameter estimation result.

In order to predict values using those parameter, we have to define the prediction function similiar to log-likelihood function above. Then we used this function to calculate fitted values on test dataset.
...
RMSE on the fitted values of home and away model are 1.148728 and 0.972111. Histogram of fitted value indicates the similiar distribution as the **RMScore_H** and **RMScore_A**.
...
* Used the normal time parameter $T$ not $\sqrt{T}$ and I found that current minute should be give more weight at the beginning of the match since we are try to predict remaining goals.
* I put all factors individually into the model and got a overfitted model with very high RMSE on the test dataset.
* I tried use *optim()* function and sometime I cannot reach convergence of my log-likelihood function.
...
I found a interesting behavior when I examing the data. Let's take a look at the distribution of the time of goals
...
Currently I used MLE estimate for parameters. However, the optimization algorithm in R may crash, or it can get stuck at a local optimum and cannot converge to a value, espically when given more data or try to build more complex models. In such cases, MCMC will give the flexibility – we can modify our models as much as you want and still effectively fit them.
